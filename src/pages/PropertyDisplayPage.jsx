...
const PropertyDisplayPage = () => {
  ...
  useEffect(() => {
    ...
    // Fetch user's favorites so the heart knows if it should be red
    fetchFavorites(); 
  }, [id, fetchFavorites]);
    ...
    <div className="min-h-screen bg-slate-50 pt-20 pb-12 px-4 md:px-10 mt-6 relative">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                ...
                <div className="flex gap-4 overflow-x-auto pb-2 no-scrollbar">
                  {property.images.map((img, index) => (
                    <img 
                      ...
                      className={`shrink-0 h-24 w-32 object-cover rounded-lg cursor-pointer border-2 transition ${mainImage === img ? 'border-[#001A33]' : 'border-transparent hover:border-slate-300'}`}
                    />
                  ))}
                </div>
              )}
              ...
              <div className="flex items-center gap-6 mt-4 pt-4 border-t border-slate-100 px-2">
                <button 
                  ...
                  className={`flex items-center gap-2 font-bold transition group ${isLiked ? 'text-red-500' : 'text-slate-500 hover:text-red-500'}`}
                >
                ...
                <button className="flex items-center gap-2 text-slate-500 hover:text-blue-600 font-bold transition group">
                  <div className="p-2 bg-slate-50 rounded-full group-hover:bg-blue-50 transition">
                    <Share2 size={20} />
                  </div>
                  Share
                </button>
          ...
          {/* RIGHT COLUMN: Price, Details & Contact */}
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200 sticky top-24">
              ...
              <div className="grid grid-cols-2 gap-4 py-6 border-y border-slate-100 mb-6">
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-slate-500"><Bed size={18} /> <span className="text-xs font-bold uppercase tracking-wider">Bedrooms</span></div>
                  <span className="font-extrabold text-slate-900">{property.residentialDetails?.bedrooms || parseInt(property.bedrooms) || 'N/A'}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-slate-500"><Bath size={18} /> <span className="text-xs font-bold uppercase tracking-wider">Bathrooms</span></div>
                  <span className="font-extrabold text-slate-900">{property.residentialDetails?.bathrooms || '2'}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-slate-500"><Maximize size={18} /> <span className="text-xs font-bold uppercase tracking-wider">Carpet Area</span></div>
                  <span className="font-extrabold text-slate-900">{property.residentialDetails?.carpetArea ? `${property.residentialDetails.carpetArea} sqft` : 'N/A'}</span>
                </div>
                <div className="flex flex-col gap-1">
                  <div className="flex items-center gap-2 text-slate-500"><Building size={18} /> <span className="text-xs font-bold uppercase tracking-wider">Floor</span></div>
                  <span className="font-extrabold text-slate-900">{property.residentialDetails?.floorNumber || property.floor || 'N/A'}</span>
                </div>
              </div>
              ...
              <button 
                ...
                className="w-full h-12 bg-[#001A33] text-white rounded-xl font-bold hover:bg-[#13304c] transition flex justify-center items-center gap-2"
              >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden relative animate-in zoom-in-95 duration-200">
            ...
            <button 
              ...
              className="absolute top-4 right-4 p-1 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-full transition"
            >
              <X size={20} />
            </button>
            ...
            <div className="p-6 md:p-8">
              ...
              {contactSuccess ? (
                <div className="py-8 flex flex-col items-center justify-center text-center space-y-3">
                  ...
                  <p className="text-sm text-slate-500">The seller has received your details and will contact you shortly.</p>
                </div>
              ) : (
                <form onSubmit={handleContactSubmit} className="space-y-4">
                  <div>
                    ...
                    <input 
                      ...
                      className="w-full h-11 border border-slate-300 rounded-lg px-3 focus:ring-2 focus:ring-[#001A33] focus:outline-none" 
                    />
                  </div>
                  ...
                  <div className="pt-2 space-y-3">
                    <label className="flex items-start gap-3 cursor-pointer group">
                      <div className="relative flex items-center justify-center mt-0.5">
                        <input 
                          type="checkbox" 
                          checked={contactForm.wantVisit}
                          onChange={(e) => setContactForm({...contactForm, wantVisit: e.target.checked})}
                          className="peer appearance-none w-5 h-5 border-2 border-slate-300 rounded bg-white checked:bg-[#001A33] checked:border-[#001A33] transition-colors cursor-pointer" 
                        />
                        <CheckCircle size={14} className="absolute text-white opacity-0 peer-checked:opacity-100 pointer-events-none" strokeWidth={3} />
                      </div>
                      <span className="text-sm font-medium text-slate-600 group-hover:text-slate-900 transition">I am interested in a site visit</span>
                    </label>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer group">
                      ...
                      <span className="text-sm font-medium text-slate-600 group-hover:text-slate-900 transition">I agree to the <span className="text-blue-600 hover:underline">Terms & Conditions</span></span>
                    </label>
                  </div>
                  ...
                  <button 
                    ...
                    className="w-full h-12 bg-[#001A33] text-white rounded-xl font-bold hover:bg-[#13304c] transition mt-6 disabled:opacity-70"
                  >
