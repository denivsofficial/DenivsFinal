...
import { MapPin, Bed, Bath, Maximize, ArrowLeft, Phone, CheckCircle, Building, Heart, Share2, X, Wallet, Wrench, Loader2 } from 'lucide-react';
...
const PropertyDisplayPage = () => {
  ...
  const handleContactSubmit = async (e) => {
    ...
    try {
      // Format the message with site visit request if checked
      let finalMessage = contactForm.message || '';
      if (contactForm.wantVisit) {
        finalMessage = `[Requested Site Visit] ${finalMessage}`.trim();
      }

      // Call backend API (if your route prefix in app.js is different, e.g., /leads, change it here)
      const response = await apiClient.post('/leads', {
        propertyId: id,
        name: contactForm.name,
        contactNumber: contactForm.phone,
        message: finalMessage || "I am interested in this property."
      });
      ...
      if (response.data.success) {
        ...
        setTimeout(() => {
          ...
          setContactForm({ name: '', phone: '', message: '', wantVisit: false, agreeTerms: false });
        }, 2500);
      }
    } catch (err) {
              ...
              {contactSuccess ? (
                ...
                <form onSubmit={handleContactSubmit} className="space-y-4">
                  <div>
                    ...
                    <input type="text" required value={contactForm.name} onChange={(e) => setContactForm({...contactForm, name: e.target.value})} placeholder="e.g. John Doe" className="w-full h-11 border border-slate-300 rounded-lg px-3 focus:ring-2 focus:ring-[#001A33] focus:outline-none" />
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-slate-700 block mb-1">Contact Number</label>
                    <input type="tel" required value={contactForm.phone} onChange={(e) => setContactForm({...contactForm, phone: e.target.value.replace(/\D/g, '')})} maxLength={10} placeholder="e.g. 9876543210" className="w-full h-11 border border-slate-300 rounded-lg px-3 focus:ring-2 focus:ring-[#001A33] focus:outline-none" />
                  </div>
                  
                  {/* Added Message Field */}
                  <div>
                    <label className="text-sm font-semibold text-slate-700 block mb-1">Message (Optional)</label>
                    <textarea value={contactForm.message} onChange={(e) => setContactForm({...contactForm, message: e.target.value})} placeholder="Any specific questions for the seller?" rows={2} className="w-full border border-slate-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#001A33] focus:outline-none resize-none" />
                  </div>
                  ...
                  <button type="submit" disabled={isSubmittingContact} className="w-full h-12 flex items-center justify-center gap-2 bg-[#001A33] text-white rounded-xl font-bold hover:bg-[#13304c] transition mt-6 disabled:opacity-70">
                    {isSubmittingContact ? <Loader2 size={18} className="animate-spin" /> : null}
