                ...
                {isLoading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        {displayData.map((item) => {
                            ...
                            return (
                                <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden group transition-all duration-300 hover:shadow-xl">
                                    <div className="relative h-56 overflow-hidden">
                                        ...
                                        <button onClick={() => toggleFavorite(item.id)} className={`absolute top-4 right-4 p-2 backdrop-blur-lg rounded-full border transition ${isLiked ? 'bg-red-50/90 border-red-200 text-red-500' : 'bg-white/20 border-white/30 text-white hover:bg-white/40'}`}>
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 backdrop-blur-[2px] pointer-events-none">
                                            <Link to={`/property/${item.id}`} className="bg-blue-950 text-white px-6 py-2 rounded-full font-bold text-sm shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 pointer-events-auto">
                                                View Property
                                            </Link>
                                        </div>
                                    </div>
