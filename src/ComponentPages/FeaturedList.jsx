...
const FeaturedList = () => {
  ...
  useEffect(() => {
    fetchProperties();
    // 🚀 FIX: Empty dependency array breaks the infinite loop
  }, []); 
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex overflow-x-auto gap-6 pb-10 no-scrollbar snap-x snap-mandatory scroll-smooth">
          ...
          {isLoading ? (
            ...
            properties.map((item) => (
              <div key={item.id} className="min-w-[85vw] sm:min-w-[300px] md:min-w-[350px] max-w-[350px] snap-center bg-white rounded-3xl shadow-md border border-slate-100 overflow-hidden group transition-all duration-300 hover:shadow-xl">
                <div className="relative h-56 overflow-hidden">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity bg-black/30 backdrop-blur-[2px]">
                      <Link to={`/property/${item.id}`} className="bg-blue-950 text-white px-6 py-2 rounded-full font-bold text-sm shadow-2xl transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300">
                          View Property
                      </Link>
                  </div>
                </div>
